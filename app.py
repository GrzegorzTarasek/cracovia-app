...
# ============================================================
    ...
    with tab_global_pii:
        ...
        player = st.selectbox("Zawodnik", players_list, key="ou_global_pii_player")
        ...
        if player_pii_all.empty:
            st.info("Brak danych PII zawodnika.")
            st.stop()

        st.markdown("### Widok 1 – wybrany okres pomiarowy")
        ...
        team_pii = (
            ...
            .median()                                # << zmiana mean → median
            .reset_index()
            .rename(columns={"PlayerIntensityIndex": "PII_team"})
        )

        comp = team_pii.copy()
        comp["PII_player"] = player_median_pii
        ...
        st.dataframe(
            comp[["Team", "PII_player", "PII_team", "diff_abs", "diff_pct", "Status"]]
            .rename(columns={
                "Team": "Zespół",
                "PII_player": "PII zawodnika (mediana)",
                "PII_team": "PII zespołu (mediana)",
    ...
    # ==================================================================
    with tab_metrics:
        st.subheader("Metryki per minuta względem zespołu (MEDIANY)")
        ...
        metrics_pick = st.multiselect("Metryki", metrics, default=metrics)

        if not metrics_pick:
            st.info("Wybierz metryki.")
            st.stop()

        # mediany zawodników
        pm = df2.groupby(["Team", "Name"])[metrics_pick].median().reset_index()
        ...
        for _, row in merged.iterrows():
            for m in metrics_pick:
                p = row[m]
                t = row[m + "_team"]
                if pd.isna(p) or pd.isna(t) or t == 0:
                    continue
                rows.append({
                    "Team": row["Team"],
                    "Player": row["Name"],
        ...
        st.dataframe(
            out.rename(columns={
                ...
                "Player": "Zawodnik",
                "Metric": "Metryka",
                "Player_median": "Mediana zawodnika",
                "Team_median": "Mediana zespołu",
                "diff_abs": "Różnica bezwzględna",
                "diff_pct": "Różnica (%)",
            })
            .round(2),
    ...
    # ==================================================================
    # TAB 3 – OKRESY POMIAROWE (MEDIANY)
    # ==================================================================
    with tab_periods:
        st.subheader("PII – Over/Under w okresach (MEDIANY)")
        ...
        option = st.selectbox("Okres", periods["Label"].tolist(), key="ou_period_select")

        sel = periods[periods["Label"] == option].iloc[0]
        d_start, d_end = sel["DateStart"], sel["DateEnd"]

        df_period = df_pii[(df_pii["DateStart"] == d_start) & (df_pii["DateEnd"] == d_end)].copy()

        # MEDIANY PII zawodników
        player_pii = (
            df_period.groupby(["Team", "Name"])["PlayerIntensityIndex"]
            .median()                                   # << zmiana mean → median
            .reset_index()
        ...
        # MEDIANY PII zespołów
        team_pii = (
            df_period.groupby("Team")["PlayerIntensityIndex"]
            .median()                                   # << zmiana mean → median
            .reset_index()
            .rename(columns={"PlayerIntensityIndex": "PII_team"})
        )

        merged = player_pii.merge(team_pii, on="Team", how="left")

        merged["diff_abs"] = merged["PII_player"] - merged["PII_team"]
        merged["diff_pct"] = (merged["PII_player"] / merged["PII_team"] - 1) * 100
        ...
        merged["Status"] = merged["diff_pct"].apply(
        ...
        st.dataframe(
            merged.rename(columns={
                "Team": "Zespół",
                "Name": "Zawodnik",
                "PII_player": "Mediana PII zawodnika",
